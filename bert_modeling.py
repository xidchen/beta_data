  ...
  """
    ...
    def call(self, inputs, **kwargs):
        ...
        if attention_mask is not None:
            ...
            # and 0.0 for masked positions,
            # this operation will create a tensor which is 0.0
            # for positions we want to attend
            # and -10000.0 for masked positions.
            adder = (1.0 - tf.cast(
                attention_mask, attention_scores.dtype)) * -10000.0
  ...
  """
    ...
    """
        if self.backward_compatible:
            ...
            bias = (tf.keras.backend.reshape(
                self.bias, self.bias_shape) if self.use_bias else None)
        else:
            kernel = self.kernel
            bias = self.bias
...
class TransformerBlock(tf.keras.layers.Layer):
  ...
  It has two sub-layers. The first is a multi-head self-attention mechanism,
  and the second is a positionwise fully connected feed-forward network.
  """
    ...
    def __init__(self, hidden_size=768, num_attention_heads=12,
        ...
        if self.hidden_size % self.num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple "
                "of the number of attention heads (%d)"
